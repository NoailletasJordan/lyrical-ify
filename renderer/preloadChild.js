...
ipcRenderer.on('launch-search', (event, args) => {
  ...
  document.querySelector('.gLFyf').value = `genius lyrics ${nameFormatter(
    args.name,
  )}${args.artistsMax2}`
...
ipcRenderer.on('sendbackhtml', (event, arg) => {
  ...
  // Remove page adds
  document
    .querySelectorAll('div[data-exclude-from-selection]')
    .forEach((e) => e.replaceWith(document.createElement('br')))
